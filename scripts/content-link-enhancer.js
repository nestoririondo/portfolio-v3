...
class ContentLinkEnhancer {
  constructor() {
    ...
    this.linkMappings = {
      ...
      "Berlin Startup Ecosystem":
        "https://www.berlin.de/sen/web/en/business/startup-ecosystem/",
  ...
  enhanceContent(content) {
    ...
    const sortedMappings = Object.entries(this.linkMappings).sort(
      ([a], [b]) => b.length - a.length
    );

    for (const [term, url] of sortedMappings) {
      // Skip if we've already linked this term or it matches exclude patterns
      if (linkedTerms.has(term.toLowerCase()) || this.shouldExclude(term))
        continue;
      ...
      if (match && !this.isInLinkContext(enhancedContent, match.index)) {
        ...
        enhancedContent =
          enhancedContent.substring(0, match.index) +
          replacement +
  ...
  addInternalLinks(content) {
    ...
    const sortedInternalMappings = Object.entries(internalLinkMappings).sort(
      ([a], [b]) => b.length - a.length
    );
    ...
    for (const [term, url] of sortedInternalMappings) {
      ...
      if (
        regex.test(enhancedContent) &&
        !enhancedContent.includes(`[${term}]`)
      ) {
