...
function generateSmartSearchTerms(topic) {
  ...
  const businessTerms = topic
    .toLowerCase()
    .replace(/\b(for|berlin|german|germany|website|web|development|how|to|the|in|with|and|or|24|12|weeks)\b/g, "")
    .trim()
    .split(/\s+/)
    ...
    try {
      ...
      if (smartResult.photos && smartResult.photos.length > 0) {
        ...
        smartResult.photos.forEach((photo, i) => {
          console.log(`   ${i+1}. ${photo.alt?.substring(0, 60) || 'Professional image'}...`);
        });
      }
    } catch (error) {
