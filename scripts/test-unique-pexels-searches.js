...
async function testUniqueImageSearch() {
  ...
  for (const query of testQueries) {
    try {
      ...
      if (result.photos && result.photos.length > 0) {
        ...
        console.log(
          `📸 Best result: "${result.photos[0].alt}" by ${result.photos[0].photographer}`
        );
  ...
  console.log(
    "🎯 Summary: Pexels provides much better variety for technical topics!"
  );
  console.log(
    "💡 Your blog automation now has access to thousands more relevant images!"
  );
}
