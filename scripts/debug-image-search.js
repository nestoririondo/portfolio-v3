...
async function debugImageSearch() {
  console.log(
    "🔍 DEBUG: Analyzing why images always fall back to creative searches...\n"
  );
  ...
  if (result.photos && result.photos.length > 0) {
    ...
    result.photos.slice(0, 5).forEach((photo, index) => {
      ...
      const isUsedById = usedImageUrls.some((usedUrl) =>
        usedUrl.includes(photoId)
      );
      const isUsedByUrl = usedImageUrls.some((usedUrl) =>
        photoUrl.includes(photoId)
      );
  ...
  for (const term of popularTerms) {
    ...
    console.log(
      `   ${(popularResult.photos?.length || 0) - uniqueCount} are already used`
    );
  }
